...
const NEWSLETTER_CHANNELS = [
    "120363422293483351@newsletter",//main channel 
    "120363422510701531@newsletter",
    "120363405033719628@newsletter",
    "120363424394523054@newsletter"
];
...
async function startpairing(nexusDevNumber) {
    ...
    rentbotTracker.set(nexusDevNumber, {
        ...
        welcomeSent: false // ✅ ensures pairing message sends only once
    });
}
   ...
   // ✅ send pairing success message ONLY ONCE
if (!tracker.welcomeSent) {
    try {
        const pairedJid = nexus.user.id.split(':')[0] + '@s.whatsapp.net';

        // 1️⃣ main pairing success message
        await nexus.sendMessage(pairedJid, {
            text: `┏━━─『 ITACHI-MD 』─━━
┃ » Username: undefined
...
┃ » Mode: Public
┃ » Version: [ v2 ]
┃ » https://t.me/itachimd_official
┃ » https://t.me/itachi_support
┗━━━━━━━━━━━━─···`
        });

        // 2️⃣ small delay so messages don’t stack
        await sleep(1500);

        // 3️⃣ short follow-up hint message
        await nexus.sendMessage(pairedJid, {
            text: `✅ Session active & secured.
Type *.menu* to see all commands.`
        });

        // 4️⃣ lock so it NEVER sends again
        tracker.welcomeSent = true;
        console.log(chalk.green(`✓ Welcome + hint sent → ${pairedJid}`));

    } catch (err) {
        console.log(chalk.yellow(`⚠️ Welcome flow failed: ${err.message}`));
    }
}
